...
const CVPreview = ({ user, aiData, language = "tr" }) => {
  ...
  const formatSkillName = (skill) => {
    ...
    if (corrections[lowerSkill]) {
      ...
      if (formatted === formatted.toLowerCase() && formatted.length > 0) {
          ...
          <div className="section-content">
            {educationList.map((edu, index) => (
              <div key={index} className="education-item">
                <div className="row-space-between">
                  <div className="university">{edu.schoolName || edu.university || edu.educationSchool || "Belirtilmemiş"}</div>
                  <div className="education-dates">
                    {formatDate(edu.startYear || edu.startDate)} - {formatDate(edu.graduationYear || edu.endDate, edu.isOngoing)}
                  </div>
                </div>
                <div className="degree">
                   {edu.degree} {edu.field || edu.department ? `, ${edu.field || edu.department}` : ""}
                </div>
              </div>
            ))}
          </div>
